...
internal class UpdateCarBrandCommandHandler(ICarBrandWriteRepository carBrandWriteRepository, IUnitOfWork unitOfWork)
    : ICommandHandler<UpdateCarBrandCommand, CarBrandId>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var brand = await carBrandWriteRepository.Get(request.Id, cancellationToken);
        var brandWithSameName = await carBrandWriteRepository.FindByName(request.Name, cancellationToken);
        if (brand is null)
        {
